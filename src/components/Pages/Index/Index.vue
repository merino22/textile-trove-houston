        ...
        <div class="p-8">
          <div class="grid lg:grid-cols-2 sm:grid-cols-1 gap-4 pb-5">
            <div class="grid grid-cols-1">
              ...
              <input v-model="name" maxlength="50" placeholder="Your Name*" name="name" type="text" class="bg-zinc-500 placeholder:text-gray-300 placeholder:italic" required />
            </div>
            <div class="grid grid-cols-1">
              <label for="email" :class="{ [`text-red-400`]: error }">E-mail <span v-if="error "> - {{ msg.email }}</span></label>
              <input v-model="email" name="email" type="text" class="bg-zinc-500 placeholder:italic placeholder:text-gray-300" placeholder="Your E-mail*" maxlength="50" required />
            </div>
          </div>
          <div class="grid grid-cols">
            <label for="message">Message</label>
            <textarea id="message" name="message" rows="5" class="bg-zinc-500 placeholder:italic placeholder:text-gray-300" placeholder="Type a message..." />
          </div>
...
export default {
  ...
  methods: {
    sendEmail() {
      emailjs.sendForm('service_f8x7s1s', 'template_nrvayeb', this.$refs.form, '4d3uDFK1Xwjpj_LSe')
        .then((result) => {
